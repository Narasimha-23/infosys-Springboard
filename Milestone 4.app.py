...
# ======================================================
# AI POWERED SMART EMAIL CLASSIFIER – DASHBOARD
...
# ------------------ LOAD DATA ------------------
DATA_PATH = r"C:\Users\reddy\OneDrive\Desktop\DT\data\cleaned_emailclass.csv"
...
def weighted_urgency_score(text):
    score = 0
    text = text.lower()
    for k, v in urgency_weights.items():
        if k in text:
            score += v
    return score
...
def final_urgency_score(text):
    vec = vectorizer.transform([text])
    ml_score = urgency_model.predict(vec)[0] * 3
    return ml_score + weighted_urgency_score(text)
...
col1.metric("📨 Total Emails", len(df_filtered))
col2.metric("📂 Categories", df_filtered["type"].nunique())
col3.metric("⚠️ Urgent Emails", df_filtered["cleaned_text"].apply(
    lambda x: final_urgency_score(x) >= 7).sum()
)
...
fig, ax = plt.subplots()
df_filtered["type"].value_counts().plot(kind="bar", ax=ax)
plt.xlabel("Category")
plt.ylabel("Count")
st.pyplot(fig)

# ------------------ EMAIL PREDICTION ------------------
st.subheader("✉️ Test New Email")
...
if st.button("Predict"):
    ...
    score = final_urgency_score(email_input)
    level = urgency_level(score)

    st.success(f"📂 Category: {category}")
    st.warning(f"⚠️ Urgency Score: {score}")
    st.info(f"🚦 Urgency Level: {level}")

# ------------------ SIMULATED EMAIL STREAM ------------------
st.subheader("🔁 Simulated Email Stream")
...
sample_emails = [
    ...
    "Need immediate attention on system failure"
]

stream_df = pd.DataFrame({"Email": sample_emails})

stream_df["Category"] = category_model.predict(
    vectorizer.transform(stream_df["Email"])
)

stream_df["Urgency Score"] = stream_df["Email"].apply(final_urgency_score)
stream_df["Urgency Level"] = stream_df["Urgency Score"].apply(urgency_level)

st.dataframe(stream_df)

st.caption("✅ Milestone 4: Dashboard, Deployment Ready & Email Stream Tested")
